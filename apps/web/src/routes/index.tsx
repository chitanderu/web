  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] p-4">
      ...
      <div className="mt-8 flex flex-col gap-4 lg:grid lg:grid-cols-2">
        <div>
          <h3 className="my-[10px] font-light text-current/80">一言</h3>

          <Hitokoto />
        </div>
        <div>
          <h3 className="my-[10px] font-light text-current/80">今日诗句</h3>
          <ShiJu />
        </div>
      </div>
...
const UserLoginStat = () => {
  const { data: user, isLoading } = useQuery({
    queryKey: ['user'],
    queryFn: () => apiClient.owner.getOwnerInfo(),
  })
  if (isLoading) return <div className="loading loading-dots" />
  if (!user) return null
  return (
    <div>
      <h3 className="mb-4 text-xl font-light text-current/80">登录记录</h3>
      <p className="relative -mt-2 mb-3 text-neutral-7">
        ...
        <span>
          上次登录时间:{' '}
          {user.lastLoginTime ? (
            <time>{formatDateTime(user.lastLoginTime)}</time>
          ) : (
            'N/A'
          )}
        </span>
