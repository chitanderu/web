          ...
          <div className="absolute inset-x-0 bottom-0 @container">
            <div className="not-prose hidden w-full translate-y-full rounded-b-lg bg-black/70 p-3 font-sans text-sm transition-all duration-300 group-hover/image:translate-y-0 @[500px]:flex">
              <div className="relative z-10 flex w-full flex-wrap items-center justify-center gap-2 opacity-0 duration-300 group-hover/image:opacity-100">
                {exifData.device && (
                  <div className="inline-flex items-center gap-1.5 rounded-md bg-[#262626]/80 px-2.5 py-1 font-medium text-[#d4d4d4]">
                    <i className="i-mingcute-camera-line text-[#a3a3a3]" />
                    <span>{exifData.device}</span>
                  </div>
                )}
                {exifData.aperture && (
                  <div className="inline-flex items-center gap-1.5 rounded-md bg-[#262626]/80 px-2.5 py-1 font-medium text-[#d4d4d4]">
                    <TablerAperture className="size-3 text-[#a3a3a3]" />
                    <span>{exifData.aperture}</span>
                  </div>
                )}
                {exifData.iso && (
                  <div className="inline-flex items-center gap-1.5 rounded-md bg-[#262626]/80 px-2.5 py-1 font-medium text-[#d4d4d4]">
                    <CarbonIsoOutline className="size-3 text-[#a3a3a3]" />
                    <span>ISO {exifData.iso}</span>
                  </div>
                )}
                {exifData.exposureTime && (
                  <div className="inline-flex items-center gap-1.5 rounded-md bg-[#262626]/80 px-2.5 py-1 font-medium text-[#d4d4d4]">
                    <MaterialSymbolsShutterSpeed className="size-3 text-[#a3a3a3]" />
                    <span>{exifData.exposureTime}s</span>
                  </div>
                )}
                {exifData.equivalent35mmFocalLength ? (
                  <div className="inline-flex items-center gap-1.5 rounded-md bg-[#262626]/80 px-2.5 py-1 font-medium text-[#d4d4d4]">
                    <StreamlineImageAccessoriesLensesPhotosCameraShutterPicturePhotographyPicturesPhotoLens className="size-3 text-[#a3a3a3]" />
                ...
                {exifData.exposureCompensation && (
                  <div className="inline-flex items-center gap-1.5 rounded-md bg-[#262626]/80 px-2.5 py-1 font-medium text-[#d4d4d4]">
                    <MaterialSymbolsExposure className="size-3 text-[#a3a3a3]" />
                    <span>{exifData.exposureCompensation} EV</span>
                  </div>
                )}
                {exifData.dateTime && (
                  <div className="inline-flex items-center gap-1.5 rounded-md bg-[#262626]/80 px-2.5 py-1 font-medium text-[#d4d4d4]">
                    <i className="i-mingcute-time-line text-[#a3a3a3]" />
                    <span>
