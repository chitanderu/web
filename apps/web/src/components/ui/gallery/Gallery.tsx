      ...
      >
        ...
        {currentIndex > 0 && (
          <div className="pointer-events-none absolute inset-y-0 left-2 flex items-center [&_*]:duration-200">
            <MotionButtonBase
              className="border-border center pointer-events-auto flex size-8 rounded-full border bg-neutral-1 p-1 opacity-80 hover:opacity-100"
              onClick={() => {
                if (!containerRef) {
                  return
                }
        ...
        <div className={clsx('gallery-indicator', 'space-x-2')}>
          {Array.from({
            ...
            <div
              key={i}
              className={clsx(
                'size-[6px] cursor-pointer rounded-full bg-neutral-7 opacity-50 transition-opacity duration-200 ease-in-out',
