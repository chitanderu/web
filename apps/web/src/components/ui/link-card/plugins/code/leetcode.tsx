...
async function fetchLeetcodeRaw(slug: string) {
  const body = {
  ...
  return camelcaseKeys(questionData.data.question)
}
...
export const leetcodePlugin: LinkCardPlugin = {
  ...
  priority: 65,
  typeClass: 'wide',
  recentlyType: RecentlyTypeEnum.Code,
  ...
  async fetch(id: string): Promise<LinkCardData> {
    const questionTitleData = await fetchLeetcodeRaw(id)
  ...
  async fetchRawMetadata(id: string, url: string) {
    const q = await fetchLeetcodeRaw(id)
    return {
      url,
      title: q.translatedTitle || id,
      difficulty: q.difficulty || undefined,
      tags: (q.topicTags || [])
        .map((t: any) => t.translatedName || t.name)
        .filter(Boolean),
      platform: 'LeetCode',
    } satisfies CodeMetadata
  },
}
