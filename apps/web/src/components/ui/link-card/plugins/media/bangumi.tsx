import type { MediaMetadata } from '@mx-space/api-client'
...
async function fetchBangumiRaw(type: string, realId: string) {
  return fetch(`/api/bangumi/${type}/${realId}`).then((r) => r.json())
}
...
export const bangumiPlugin: LinkCardPlugin = {
  ...
  priority: 70,
  typeClass: 'media',
  recentlyType: RecentlyTypeEnum.Media,
  ...
  async fetch(id: string): Promise<LinkCardData> {
    const [type, realId] = id.split('/')
    const json = await fetchBangumiRaw(type, realId)
  ...
  async fetchRawMetadata(id: string, url: string) {
    ...
    if (type === 'subject') {
      title = json.name_cn || json.name || ''
      originalTitle =
        json.name_cn && json.name_cn !== json.name ? json.name : undefined
    }
    ...
    return {
      ...
      title,
      originalTitle,
      cover: json.images?.grid || undefined,
      rating: json.rating?.score || undefined,
      description: json.summary ? json.summary.slice(0, 200) : undefined,
      genre: 'Anime',
