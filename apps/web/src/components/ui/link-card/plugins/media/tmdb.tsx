...
async function fetchTmdbRaw(type: string, realId: string) {
  ...
  return fetch(`/api/tmdb/${type}/${realId}?language=${userLanguage}`).then(
    (r) => r.json(),
  )
}
...
export const tmdbPlugin: LinkCardPlugin = {
  ...
  async fetch(id: string): Promise<LinkCardData> {
    const [type, realId] = id.split('/')
    const json = await fetchTmdbRaw(type, realId)
  ...
  async fetchRawMetadata(id: string, url: string) {
    ...
    return {
      url,
      title: title || '',
      originalTitle: title !== originalTitle ? originalTitle : undefined,
      cover: json.poster_path
        ? `https://image.tmdb.org/t/p/w500${json.poster_path}`
        : undefined,
      rating: json.vote_average || undefined,
      description: json.overview || undefined,
      genre: type === 'tv' ? 'TV' : 'Movie',
