...
export const ComposedKBarProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const actions: Action[] = useRefValue(
    () =>
      routes
        .map<Action | null>(({ path, config: { title, redirect } }) => {
          ...
          return {
            ...
            name,
            perform: () => {
              navigate(`/dashboard${redirect || path}`)
            },
          }
        })
  ...
  return (
    <KBarProvider actions={actions}>
      ...
      <KBarPortal>
        <KBarPositioner>
          <KBarAnimator className="w-full max-w-[600px] overflow-hidden rounded-lg bg-neutral-1/60 text-neutral-9 shadow-xl backdrop-blur-md">
            <KBarSearch className="w-full border-none bg-neutral-2/30 p-3 text-lg text-neutral-9 outline-hidden backdrop-blur-md" />
            <RenderResults />
          </KBarAnimator>
        </KBarPositioner>
      </KBarPortal>
      ...
      {action.shortcut?.length ? (
        <div aria-hidden className="grid grid-flow-col gap-1">
          {action.shortcut.map((sc) => (
            <kbd className="rounded bg-neutral-1/10 p-1 text-sm" key={sc}>
              {sc}
            </kbd>
          ))}
        </div>
