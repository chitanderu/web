  ...
  return (
    <>
      {isLoading || !categoryId ? (
        <div className="h-2 w-[120px] animate-pulse bg-neutral-1" />
      ) : (
        <label className="text-neutral-9">{`${webUrl}/posts/${categoryIdMap?.[categoryId]?.slug}/`}</label>
      )}

      <div className="relative ml-1 inline-flex min-w-8 items-center overflow-hidden rounded-md bg-neutral-1 py-1 [&_*]:leading-4">
        <input
          className="input input-sm absolute w-full translate-y-px border-0! bg-transparent outline-hidden!"
          value={slug}
          onChange={(e) => {
