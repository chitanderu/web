...
import { useWebglFlagAtom } from '~/atoms'
...
export const ImageDetailSection: FC<ImageDetailSectionProps> = (props) => {
  ...
  // GPU 加速 flag
  const [useWebgl, setUseWebgl] = useAtom(useWebglFlagAtom)

  const [loading, setLoading] = useState(false)
  const handleCorrectImageDimensions = useEventCallback(
    async (images: ArticleImage[]) => {
      if (loading) return
      setLoading(true)
            ...
            new Promise<ArticleImage>((resolve, reject) => {
              ...
              $image.onload = () => {
                try {
                  ...
                  // 如果 WebGL 失败，回退到普通方式
                  if (useWebgl && err instanceof Error) {
                    try {
      ...
      <div className="relative flex w-full flex-col">
        <div className="flex items-center justify-between space-x-2">
          ...
          <StyledButton
            ...
            disabled={loading}
            variant="secondary"
            onClick={() => handleCorrectImageDimensions(nextImages)}
          >
            {loading && <i className="loading loading-spinner loading-sm" />}
            刷新图片信息
          </StyledButton>
        </div>
        <div className="mt-2 flex items-center gap-2 pl-1 text-sm">
          <CheckBoxLabel
            checked={useWebgl}
            label="使用 GPU 加速"
            onCheckChange={setUseWebgl}
          />
          <span className="text-neutral-8/60">
            （实验性功能，使用 WebGL 计算 BlurHash）
          </span>
        </div>
        <div className="my-2 flex flex-col gap-2">
          {nextImages.map((image) => (
            <Collapse className="mt-4" key={image.src} title={image.src}>
